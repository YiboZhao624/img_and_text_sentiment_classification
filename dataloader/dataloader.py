...
class Dataloader(AbstractDataLoader):
    def __init__(self, config, dataset):
        ...
        self.embedding_from_pretrain =config["embedding_from_pretrain"]
        try:
            ...
            if self.embedding_from_pretrain:
                self.embedder = BertModel.from_pretrained('bert-base-uncased',output_hidden_states = True).to(self.device)
                self.embedder.eval()
        ...
        if not self.embedding_from_pretrain:
    ...
    def __build_batch(self,batch_data):
        ...
        text_inputs = self.truncate_tensor(source_index_batch)#(batchsize,maxlen)
        if self.embedding_from_pretrain:
            text_vector = self.convert_ids_to_vector(text_inputs['sequence'].to(self.device))
        else:
            text_vector = None
        ...
        if tag_batch[0] != None:
            return {
                "id":ids,
                "source_text_vector":text_vector,
                "source_mask_vector":text_inputs['mask'],
                "source_text_batch":source_text_batch,
                "source_index_batch":text_inputs['sequence'],
    ...
    def __init_batches(self):
        ...
        print("building dataloader from dataset,it takes some time……")
    ...
    def convert_ids_to_vector(self,tokens_tensor):
        ...
        toreturn = torch.tensor([item.cpu().detach().numpy() for item in batch_vector])
        return toreturn
    ...
    def truncate_tensor(self, sequence):
        ...
        for instance in sequence:
            if len(sequence) > max_len:
                print("it is larger than input len")
        ...
        for instance in sequence:
            ...
            len_diff = self.max_input_len - len(one_tag_list)
