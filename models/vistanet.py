...
class VggNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # No grad for fixed feature layers
        with torch.no_grad():
        ...
        # Classifier has grad
        x = self.model.classifier(pooled)
...
class Image_Text_Attention(nn.Module):
    ...
    def forward(self, image_emb, seq_emb, mask):
        ...
        if self.l is None or self.k is None:
            ...
            self.img_layer = nn.Linear(image_emb.size(2), 5).to(self.device)  # Mapping image_emb to 50 dimension
            self.seq_layer = nn.Linear(self.k, 1).to(self.device)              # Mapping seq_emb to 50 dimension

            self.V_weight = nn.Parameter(torch.Tensor(self.l, self.l)).to(self.device)
            self.init_weights()

        # Linear mapping
        p = self.img_layer(image_emb)  #(bs,1,20) 
        p = p.transpose(1,2)           #(bs,20,1) 将图像信息分配到20个空间中
        q = self.seq_layer(seq_emb)    #(bs,200,1)
        
        # Inner product + mapping (calculation of score)
        emb = torch.matmul(p, q.transpose(1,2))
           # （bs,20,200)
        #print('emb(before add):',emb.shape)
        emb = emb + q.transpose(1, 2)              # [bs, 20, 200=L]
        #print('emb(after add):',emb.shape)
        emb = torch.matmul(emb, self.V_weight)     # [bs, 20, 200=l]
...
class VistaNet(nn.Module):
    def __init__(self, config,dataloader):
        ...
        self.output_layer1 = nn.Linear(5*config['embedding_dim'],config['class_num'])
    ...
    def forward(self,batch):
        ...
        #print(text_inputs["sequence"].device)
        ...
        #print("doc emb:",doc_emb.shape)#(bs,1,768)
        D_emb = self.doc_self_attention(doc_emb,doc_emb,doc_emb)
        #print("\nD_emb:\n",D_emb.shape)#:(bs,20,768)
        D_emb =D_emb.contiguous().view(doc_emb.size(0),-1)
    ...
    def model_test(self,batch,dataloader,is_display=False):
        ...
        if self.device:
            image_inputs = image_inputs.to(self.device)
            #text_inputs["sequence"] = text_inputs["sequence"].cuda(self.device)
            #text_inputs["mask"] = text_inputs["mask"].cuda(self.device)
            mask = mask.to(self.device)
            text_vector = text_vector.to(self.device)
